...
2. **Account classification:** official specialist HiScore boards are probed for level-3, 1-defence, UIM, HCIM and IM classification. HCIM eligibility is validated against the current account stats so dead HCIM accounts that have progressed beyond their frozen Hardcore snapshot are classified as regular Ironman. TempleOSRS player info is always used to identify GIM accounts where possible. If the gamemode cannot be resolved, the app defaults to **Main**.
...
- Every statistic from the HiScores (Except League points and Grid Points)

Customisation options:
- **Use special EHP/EHB where applicable**: keeps the same EHP/EHB columns but substitutes the applicable special efficiency value for accounts that have one. It does not add separate Special EHP or Special EHB columns.
- HiScores, TempleOSRS, and Wise Old Man profile links.
- **Stat customisation**: chose which Skills, Bosses, Activities to include in the output.

Use **Copy** or **Download CSV** to export the currently configured output.

## Preferences and cookies

The site has no backend and does not store any data.
...
A Pages workflow in `.github/workflows/pages.yml` deploys pushes to `main` automatically through GitHub Actions.
